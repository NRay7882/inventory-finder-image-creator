...
- Fleet deploy private key at `%USERPROFILE%\.ssh\inventory_deploy` (Windows) or `~/.ssh/inventory_deploy` (Mac/Linux)
...
| `-DeployKeyPath` | `~\.ssh\inventory_deploy` | Fleet deploy private key path |
...
| `--deploy-key PATH` | `~/.ssh/inventory_deploy` | Fleet deploy private key path |
...
DEPLOY_KEY_B64=<base64-encoded private deploy key>
```
...
# Windows (PowerShell)
[Convert]::ToBase64String([IO.File]::ReadAllBytes("$env:USERPROFILE\.ssh\inventory_deploy"))
```
